...
class SlowStatuteDownloader:
    def __init__(self, delay_seconds: int = 10, output_dir: str = 'statute_html'):
        ...
        # Create subdirectories for constitution and each title
        const_dir = self.output_dir / "constitution"
    ...
    def get_filename(self, cite_id: str, title_number: int = None, doc_type: str = 'statute') -> Path:
        """Generate filename for a document"""
        if doc_type == 'constitution':
            doc_dir = self.output_dir / "constitution"
        else:
            doc_dir = self.output_dir / f"title_{title_number:02d}"
        return doc_dir / f"cite_{cite_id}.html"

    def download_statute(self, url: str, cite_id: str, title_number: int = None, doc_type: str = 'statute') -> bool:
        """Download a single document HTML page"""
        ...
        filename = self.get_filename(cite_id, title_number, doc_type)
        ...
        try:
            ...
            if title_number is not None:
                metadata['title_number'] = title_number
    ...
    def download_from_url_list(self, url_file: str = 'oklahoma_statute_urls.json'):
        ...
        for i, statute_info in enumerate(urls, 1):
            ...
            title_number = statute_info.get('title_number')  # May be None for constitution
            doc_type = statute_info.get('type', 'statute')  # Default to 'statute'

            print(f"\n[{i}/{len(urls)}] Processing cite_id: {cite_id}")

            if self.download_statute(url, cite_id, title_number, doc_type):
                success_count += 1
            else:
                failure_count += 1
