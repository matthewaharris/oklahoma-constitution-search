...
def main():
    """Main entry point"""
    print("Oklahoma Statutes URL Collector")
    print("This tool respectfully collects statute URLs from OSCN")
    print()

    # Ask user for preferences
    print("Options:")
    print("1. Collect all titles (1-85) - Takes ~15 minutes with 10-second delay")
    print("2. Collect specific title range")
    print("3. Test with single title")
    print()
    ...
    if choice == '1':
        # Collect all titles
        collector.collect_and_save(1, 85)

    elif choice == '2':
        # Custom range
        start = int(input("Start title: ").strip())
        end = int(input("End title: ").strip())
        collector.collect_and_save(start, end)
